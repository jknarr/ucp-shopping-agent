...
root_agent = Agent(
    ...
    instruction=(
        ...
        "checkout is created or updated, do not repeat its item or pricing details in "
        "prose because the client renders the authoritative checkout card. If asking a "
        "follow-up, ask only whether the buyer wants to add any other items. Do not ask for an email "
        ...
        "handler opens directly in the conversation. Describe this as selecting a "
        "payment method, never as completing a transaction; completion happens only "
        "after the buyer reviews the returned method and explicitly confirms purchase. "
        "When the buyer asks to change, "
